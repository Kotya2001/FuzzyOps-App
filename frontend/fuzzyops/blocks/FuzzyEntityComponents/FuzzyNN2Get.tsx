...
import { fuzzynn2 } from '../../http/FuzzyNNApi';


export const FuzzyNN2Get = ({ header, tag }: FuzzyProps) => {

	const { isLoadXtrain, data } = useAppSelector(state => state.FuzzyNN2Reducer);
	...
	const getParams = async () => {
		...
		try {
			const response = await fuzzynn2(data);
			...
			if (response.data.status === 200) {
				...
				anchor.href = url;
				anchor.download = 'inference.json';
				...
				<Box className={styles.block}>
					...
					<div className={styles.blockBoxHeader}>
						...
						<div className={styles.LoadContent}>
							{isLoadXtrain && <Button appearance='primary' onClick={() => getParams()} disabled={isDisabled}>Найти</Button>}
							{loading && <div className={styles.loadingMessage}>Идет вычислени...</div>}
