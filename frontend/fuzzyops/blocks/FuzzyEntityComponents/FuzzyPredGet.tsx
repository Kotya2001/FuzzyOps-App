...
import { fuzzypred } from '../../http/FuzzyNNApi';


export const FuzzyPredGet = ({ header, tag }: FuzzyProps) => {

	const { isLoadX, predData } = useAppSelector(state => state.FuzzyPredReducer);
	...
	const getParams = async () => {
		...
		try {
			const response = await fuzzypred(predData);
			...
			if (response.data.status === 200) {
				...
				anchor.href = url;
				anchor.download = 'fuzzy_reg_params.json';
				...
				<Box className={styles.block}>
					...
					<div className={styles.blockBoxHeader}>
						...
						<div className={styles.LoadContent}>
							{isLoadX && <Button appearance='primary' onClick={() => getParams()} disabled={isDisabled}>Найти</Button>}
